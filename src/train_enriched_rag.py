"""
Fine-tuning LoRA/QLoRA de Qwen2.5-VL sobre dataset_enriched con RAG-aware.
...
Entrenamiento:
    python -m src.train_enriched_rag [--epochs 1] [--dry-run]
"""
...
from src.vlm_infer_enriched import RESULTS_ROOT, SPLIT_ALIASES, load_enriched_dataset
from src.vlm_lora_training import VlmLoraTrainConfig, run_lora_training

RAG_DATASET_PATH = Path("outputs/datasets/dermavqa_iiyi_llm_synthesized_answer_finetune_rag_e5_small.jsonl")
ANSWER_COLUMNS = ("answer_es",)

RUN_DIR = RESULTS_ROOT / "vlm_lora_rag_aware"
...
TRAIN_CONFIG = VlmLoraTrainConfig(
    ...
    load_records=load_enriched_dataset,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Fine-tuning LoRA RAG-aware sobre dataset_enriched"
    )

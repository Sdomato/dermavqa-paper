...
RAG condiciona la generación en evidencia recuperada \citep{lewis2020rag}. Para similitud textual usamos TF--IDF, Sentence-BERT \citep{reimers2019sbert} y Multilingual E5 \citep{wang2024multilinguale5}. A diferencia de RAG factual sobre una base curada, aquí cada contexto pertenece a otro paciente; por ello evaluamos explícitamente cuándo ayuda y cuándo contamina la respuesta.
...
\subsection{Recuperación textual}

Los baselines recuperan del split de entrenamiento el caso cuya pregunta es más similar y devuelven su respuesta. Comparamos TF--IDF con normalización, \texttt{multilingual-e5-small} y \texttt{paraphrase-multilingual-MiniLM-L12-v2}. Para E5 usamos los prefijos \texttt{query:} y \texttt{passage:}, embeddings normalizados y similitud coseno. Validación y test nunca ingresan al índice.
...
Construimos un índice E5 sobre las 842 preguntas únicas de train. Para cada fila recuperamos los tres encuentros más cercanos y agregamos al prompt sus preguntas y respuestas. En train se excluye el propio \texttt{encounter\_id} (\emph{leave-one-out}); en validación y test se consulta exclusivamente el índice de train.
...
La comparación VLM principal se realiza sobre las mismas 314 filas de test, una por imagen. Reportamos sacreBLEU corpus \citep{post2018sacrebleu}, chrF corpus y chrF medio \citep{popovic2015chrf}, ROUGE-L \citep{lin2004rouge}, token-F1 y BERTScore F1 multilingüe \citep{zhang2020bertscore}. Las métricas de retrieval se calculan a nivel caso ($n=100$), por lo que se presentan por separado. No seleccionamos la mejor imagen de cada encuentro.
...
\subsection{Recuperación frente a generación}

La Tabla~\ref{tab:retrieval} compara retrieval y LoRA enriquecido a nivel caso. SBERT obtiene el mayor sacreBLEU, coherente con copiar una respuesta real cercana. LoRA logra mayor chrF corpus, chrF medio, ROUGE-L, token-F1 y BERTScore, lo que sugiere mejor cobertura semántica aunque menor coincidencia exacta de n-gramas.
...
\begin{tabular}{lrrrr}
\toprule
Método & BLEU & chrF$_m$ & Tok-F1 & BERT-F1 \\
\midrule
TF--IDF & 8,266 & 0,286 & 0,256 & 0,716 \\
E5-small & 8,555 & 0,308 & 0,259 & 0,714 \\
SBERT & \textbf{8,795} & 0,309 & 0,269 & 0,720 \\
LoRA VLM & 5,022 & \textbf{0,368} & \textbf{0,314} & \textbf{0,737} \\
\bottomrule
\end{tabular}
\caption{Target enriquecido a nivel caso ($n=100$). La fila LoRA agrega predicciones de las imágenes del encuentro sin selección oracle.}
\label{tab:retrieval}
\end{table}
